...
import pandas as pd
from django.shortcuts import render
from appointment.vital_analysis_model import train_ml_model, predict_risk_with_ml, generate_issues_report 
...
def vital_analysis(request):
    if request.method == 'POST':
        dataset_path = os.path.join(settings.BASE_DIR, 'appointment', 'human_vital_signs_dataset_2024.csv')
        
        if not os.path.exists(dataset_path):
            return render(request, 'appointment/vital_analysis.html', {
                'error': 'Dataset file not found. Please ensure the dataset is available.'
            })

        try:
            model = train_ml_model(dataset_path)
        ...
        try:
            report_text = generate_issues_report(risk_category)
        except Exception as e:
            return render(request, 'appointment/vital_analysis.html', {
